...
export function AcCard({
  ...
  // arc progress (16-30 °C)
  const pct = Math.min(1, Math.max(0, (temp - 16) / (30 - 16)));
  const arcLen = Math.PI * 90; // approx arc length in px for a 90 radius semicircle
  const dash = pct * arcLen;

  const accent = isOn ? "var(--active)" : "var(--muted-foreground)";

  return (
    <div className="rounded-2xl bg-card p-5 border border-border flex flex-col gap-4">
      ...
      {/* Dial */}
      <div className="relative mx-auto w-[220px] h-[140px] mt-2">
        <svg viewBox="0 0 220 140" className="w-full h-full -rotate-0">
          <path
            d="M 25 120 A 85 85 0 1 1 195 120"
            fill="none"
            stroke="var(--muted)"
            strokeWidth="10"
            strokeLinecap="round"
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pt-4">
          <div className="text-xs text-muted-foreground">{isOn ? modeLabel : "Off"}</div>
          <div className="text-4xl font-light tabular-nums">
            {temp}
            <span className="text-base align-top text-muted-foreground ml-0.5">°C</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-4">
        <button
          ...
          className="size-11 rounded-full border border-border flex items-center justify-center hover:bg-accent transition"
          aria-label="Decrease"
        >
...
function Selector({
  ...
  return (
    <div className="relative">
      <button
        onClick={onToggle}
        className="w-full rounded-xl bg-surface-elevated px-3 py-2.5 flex items-center gap-2 hover:bg-accent transition text-left"
      >
        ...
        <div className="absolute z-20 left-0 right-0 mt-2 rounded-xl bg-popover border border-border shadow-xl py-1 overflow-hidden">
          {options.map((o) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 text-sm hover:bg-accent transition ${
